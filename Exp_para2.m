...
for k = 1:length(Pinun_pool)
    ...
    [P2max_Pin(k),max_k(k)] = max(a2sq);
    P2max_Pin(k) = P2max_Pin(k).*ke2_assume;
    max_lamb(k) = lambin_pool(max_k(k));
    intra_P(k) = a1sq_solu(max_k(k));
    intra_ref(k) = a1sq_solu(1020);
